...
public:
	...
	__device__ vec3 getdirection() const { return direction; }
	__device__ vec3 getorigin() const { return origin; }
};
...
__device__ inline ray getrefractedray(const vec3& incident, const vec3& normal, const vec3& pos, const float& ratio) {
	...
	vec3 dir = c > 0.0f ? ratio * incident + (ratio * r - sqrt(c)) * normal : getreflectedray(incident, normal, pos).getdirection();
	return ray(pos, dir);
}
